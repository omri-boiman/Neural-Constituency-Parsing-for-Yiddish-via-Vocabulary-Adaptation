...
# ================= CONFIGURATION =================
# 🟢 ENCODER: Using the Kulick Base Model
ENCODER_PATH = 'skulick/xlmb-ybc-ck05'

# 🟢 OUTPUT: New folder for Scalar Mixing run
OUTPUT_DIR = './output/phase3_parser_scalar_kulick'
MODEL_FILE = os.path.join(OUTPUT_DIR, 'yiddish_parser_scalar.pt')
    ...
    class FrozenModel(OriginalModel):
        def __init__(self, *args, **kwargs):
            ...
            if count > 0:
                ...
                print(f"   ❌ ERROR: Could not find any embeddings to freeze!")
...
def train():
    ...
    args = {
        'train': os.path.join(DATA_DIR, 'train_aug.txt'),
        ...
        # 🟢 SCALAR MIXING CONFIGURATION (Changed from Mean)
        'finetune': False,           # Keep weights frozen (Memory Efficient)
        'n_bert_layers': 12,         # Listen to ALL 12 layers (Crucial for Scalar Mix)
        'bert_pooling': 'scalar_mix',# The Strategy: Learn weighted average
        'mix_dropout': 0.0,
    ...
    print(f"🚀 Starting SCALAR MIXING Training (Kulick Base): {ENCODER_PATH}")
    print(f"   Pooling: scalar_mix (All 12 Layers)")
